...
| B7 | Event/notification backbone + topics | **RESOLVED (2026-06-01, from running regtest)** | Eventing is **Kafka** (Redpanda `v25.2.1` on `:9092`). `rpk topic list` on the live stack shows topics: **`blocks-final`** (1p) — new finalized blocks (our new-block signal); **`txmeta`** (32p) — tx metadata (our new-tx signal); `invalid-blocks` (1p), `invalid-subtrees` (32p), `rejectedtx` (1p). The `services-go` relay subscribes to `blocks-final` (reorg/confirmation) and `txmeta`. Re-confirm topic names per node minor at build. |
| B8 | Latest release tag | **RESOLVED** | **v0.15.1** (2026-05-22), 32 releases total. Source: github.com/bsv-blockchain/teranode releases. |
| B9 | Exact Docker image ref/tag | **RESOLVED (2026-06-01, from cloned quickstart)** | `compose/docker-teranode.yml`: **`ghcr.io/bsv-blockchain/teranode:${TERANODE_VERSION:-v0.15.1}`**; `.env.example` pins `TERANODE_VERSION=v0.15.1`. Supporting images: `postgres:17`, `redpandadata/redpanda:v25.2.1` (+ console `v3.2.2`), `ghcr.io/bsv-blockchain/aerospike-server:8.1.2.0-1`, `prom/prometheus:v2.44.0`, `grafana/grafana:12.2.0`, `nginx:1.29.2`. Exact digests pinned in `node-docker/` after pull. |
| B10 | Chronicle-current confirmation | **RESOLVED-BY-VERSION (2026-06-01)** | Running image `ghcr.io/bsv-blockchain/teranode:v0.15.1` (label `org.opencontainers.image.version=v0.15.1`, built ~2026-05-22) is **post-Chronicle** (mainnet activation 7 Apr 2026), so the binary is Chronicle-current. `teranode-cli` has no `version` subcommand (use image label). **Still to validate empirically in Phase 2c:** that signing with `SIGHASH_ALL\|FORKID` is accepted and the Chronicle relaxations (NULLDUMMY/MINIMALIF for tx version > 1) behave as documented — confirm against the regtest validator when broadcasting. |
...
| # | Item | Status | Note |
|---|---|---|---|
| E1 | **CTO substrate spec** (`CTO_BSV_Build_Spec_v1.md`) | **RESOLVED — RECONCILED (2026-06-01)** | The spec was **found** at `D:\claude\cto\spec\source\CTO_BSV_Build_Spec_v1.md` (it was not missing — earlier search scoped only this repo + the Drive). `spec/CTO-PRIMITIVES.md` is now **reconciled against it**: the blinded commitment was **corrected** to the exact CTO format `SHA-256("CTO/commit/v1" ‖ r ‖ value)` (raw concat, 32-byte `r`) across C/TS/Go (vectors regenerated, parity green); HKDF-SHA256/AES-256-GCM/secp256k1 match (CTO allows AES-GCM or ChaCha20-Poly1305 — we use AES-GCM); the TE keystone correctly uses EP3860037A1 (distinct from CTO's per-transfer OTS); TE is stricter than CTO (no OP_RETURN). Tier S ↔ `custody` pkg; Tier T (TEE) not implemented. |
| E2 | **Docker** on host | **RESOLVED (2026-06-01)** | Windows host has no Docker Desktop and no admin/UAC, but **WSL2 Ubuntu 24.04 (systemd enabled)** is present. Installed **Docker Engine 29.5.2 + Compose v5.1.4 inside WSL as root** (`wsl -u root`, no Windows admin needed); `hello-world` runs; `systemctl enable --now docker`. All node/regtest work runs via `wsl -u root --cd <path> -- ...`. Repo on Windows `D:\…` is `/mnt/d/…` in WSL. |
| E3 | **C toolchain** (gcc/clang, make) | **RESOLVABLE IN WSL (2026-06-01)** | Not on the Windows host (no admin), but **WSL2 Ubuntu can `apt-get install build-essential` as root** (same path that unblocked Docker, E2) — so the PostgreSQL-18 fork (Phase 3) and C crypto bindings are buildable in WSL. Phase 1 was built TS/Go-first per operator decision (Appendix B.1 C-parity clause stays open until the C binding lands). **Phase 3 itself is a large build** (fork+compile PG18, capture extension, te_verify, cold-rebuild); undertaking it is the next scope step. |

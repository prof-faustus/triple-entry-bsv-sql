...
| # | Item | Status | Note |
|---|---|---|---|
| E1 | **CTO substrate spec** (`CTO_BSV_Build_Spec_v1.md`) | **OPEN — BLOCKER** | Spec line 15 / `SYS-SUB-001` / `SYS-CON-007` build *on* the CTO confidential-object primitive (secp256k1 ECDH, HKDF, AEAD, SHA-256 commitments, UTXO-lineage objects, time-locked recovery, Tier F/S/T profiles). **The file is not present in this repo or on disk.** Must be supplied, or its relevant primitives explicitly re-derived and recorded here, before Phase 1 confidentiality/commitment and Phase 6 custody work can be grounded. The ECDH-HMAC keystone (§5) is grounded directly in EP3860037A1 and can proceed. |
...
| E3 | **C toolchain** (gcc/clang, make) | **OPEN — BLOCKER** | Not present → the PostgreSQL-18 C fork (`pg-fork`, Phase 3) and the C crypto bindings (Phase 1) cannot be built. `node` (v24) and `go` (1.26) are present, so the TS/Go sides can proceed. |
